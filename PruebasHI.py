...
##########################################################################################
# Corroborando la extraccion de datos
##########################################################################################

sujeto = np.random.randint(1, 16)
canal = np.random.randint(1, 7)

sujeto = '0' + str(sujeto)
ruta = os.path.dirname(os.getcwd())
ruta += '/Base_de_Datos_Habla_Imaginada/S' + sujeto + '/S' + sujeto + '_EEG.mat'
data = scipy.io.loadmat(ruta)

dato = np.random.randint(0, data['EEG'].shape[0])

valores = data['EEG'][dato][:-3][(canal-1)*4096:canal*4096]
plt.plot(np.array(range(valores.shape[0])) / 1024, valores)
plt.show()
...
# Señal de ejemplo
fs = 1024  # Hz (cambia según tus datos)
dataset = mbp.DataSetEEG(sujeto=1)
datos = dataset[2][0][4*4096:5*4096]

# le hago downsampling a 128 hz a los datos
datos = decimate(datos, 8)
fs = fs / 8
...
# Reconstrucción de cada banda
bands = []
for i in range(len(coeffs)):
    coeffs_band = [np.zeros_like(c) for c in coeffs]
    coeffs_band[i] = coeffs[i]
    band_signal = pywt.waverec(coeffs_band, wavelet)
    bands.append(band_signal[:len(x)])
...
# Graficar
fig, axes = plt.subplots(len(bands)+1, 2, figsize=(12, 2*(len(bands)+1)))

# Señal original
axes[0,0].plot(t, x)
axes[0,0].set_title("Señal original")
freqs, fft_vals = compute_fft(x, fs)
axes[0,1].plot(freqs, fft_vals)
axes[0,1].set_title("FFT señal original")

# Bandas wavelet
for i, band in enumerate(bands):
    axes[i+1,0].plot(t, band)
    axes[i+1,0].set_title(f"Banda {i} (coef {'cA' if i==0 else f'cD{level-i+1}'})")
    freqs, fft_vals = compute_fft(band, fs)
    axes[i+1,1].plot(freqs, fft_vals)
    axes[i+1,1].set_title(f"FFT Banda {i}")
    # eje y logaritmico
    #axes[i+1,1].set_yscale('log')
...
# Señal de ejemplo (EEG simulada)
fs = 256  # frecuencia de muestreo
tiempo = 10000
t = np.linspace(0, tiempo, tiempo*fs, endpoint=False)
# quiero que la señal sea ruido blanco
signal = np.random.randn(tiempo*fs)
...
for i in range(len(coeffs)):
    ...
    coeffs_band[i] = coeffs[i]
    band_signal = pywt.waverec(coeffs_band, wavelet)
    bands.append(band_signal[:len(signal)])
